...
單一 query 會命中同一批素材。產出內容前，用 2-3 個不同角度查 `/context`，再綜合使用：

- 第一次：直接主題（如「養成好習慣」）
- 第二次：換切入點（如「克服拖延 開始行動」）
- 第三次：更深層（如「複利系統 長期堅持」）

取回後去重，從不同角度的素材中挑選最豐富的組合。

**注意：每個角度都必須跟主題有邏輯關聯。** 不要為了「素材不一樣」而查無關主題，否則組裝出來會很跳。
